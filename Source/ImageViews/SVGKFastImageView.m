...
- (void)populateFromImage:(SVGKImage*) im
{
	...
	if( im == nil )
	{
		SVGKitLogError("[%@] WARNING: you have initialized an SVGKImageView with a blank image (nil). Possibly because you're using Storyboards or NIBs which Apple won't allow us to decorate. Make sure you assign an SVGKImage to the .image property!", [self class]);
	}
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	...
	{
		/*SVGKitLogError("transform changed. Setting layer scale: %2.2f --> %2.2f", self.layer.contentsScale, self.transform.a);
...
-(void)drawRect:(CGRect)rect
{
  ...
  CGSize translateSize;
   ...
   translateSize = CGSizeMake(0, (self.bounds.size.height - self.bounds.size.width)/2.0f);
	...
	//DEBUG: SVGKitLogError("cols, rows: %i, %i ... scaleConvert: %@ ... tilesize: %@", cols, rows, NSStringFromCGSize(scaleConvertImageToView), NSStringFromCGSize(tileSize) );
