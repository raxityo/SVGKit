...
- (void)populateFromImage:(SVGKImage*) im
{
	...
	if( im == nil )
	{
        ...
        SVGKitLogError("[%@] WARNING: you have initialized an [%@] with a blank image (nil). Possibly because you're using Storyboards or NIBs which Apple won't allow us to decorate. Make sure you assign an SVGKImage to the .image property!", [self class], [self class]);
#if SVGKIT_UIKIT
		...
		SVGKitLogError("About to make a blank image using the inlined SVG = %@", svgStringDefaultContents);
