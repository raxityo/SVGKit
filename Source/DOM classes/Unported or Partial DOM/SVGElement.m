...
-(void) reCalculateAndSetViewportElementReferenceUsingFirstSVGAncestor:(SVGElement*) firstAncestor
{
	...
	{
		SVGKitLogError("[%@] WARNING: setting self (tag = %@) to be a viewport", [self class], self.tagName );
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				SVGKitLogError("viewport Element = %@ ... for node/element = %@", self.viewportElement, self.tagName);
#endif
			}
		}
	}
...
- (void)postProcessAttributesAddingErrorsTo:(SVGKParseResult *)parseResult  {
	...
	if( [[self getAttribute:@"transform"] length] > 0  || [[self getAttribute:@"gradientTransform"] length] > 0)
	{
		 ...
		 ^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop)
		{
			...
			//EXTREME DEBUG: SVGKitLogError("[%@] DEBUG: found a transform element (should be command + open bracket + args + close bracket) = %@", [self class], transformString);
			...
			//EXTREME DEBUG: SVGKitLogError("[%@] DEBUG: found parameters = %@", [self class], parameterStrings);
...
//					selfTransformable.transform = CGAffineTransformConcat( nt, selfTransformable.transform ); // Apple's method appears to be backwards, and not doing what Apple's docs state
		...
		//DEBUG: SVGKitLogError("[%@] Set local / relative transform = (%2.2f, %2.2f // %2.2f, %2.2f) + (%2.2f, %2.2f translate)", [self class], selfTransformable.transform.a, selfTransformable.transform.b, selfTransformable.transform.c, selfTransformable.transform.d, selfTransformable.transform.tx, selfTransformable.transform.ty );
		}
	}
