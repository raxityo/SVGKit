			...
			if( SVGRectIsInitialized( frameViewBox ) )
			{
				...
				if( svgSVGElement.preserveAspectRatio.baseVal.meetOrSlice == SVG_MEETORSLICE_MEET ) // ALWAYS TRUE in current implementation
				{
					if( ABS( svgSVGElement.aspectRatioFromWidthPerHeight - svgSVGElement.aspectRatioFromViewBox) > 0.00001 )
					{
						...
						SVGKitLogError("ratioOfRatios = %.2f", ratioOfRatios );
						SVGKitLogError("Experimental: auto-scaling viewbox transform to fulfil SVG spec's default MEET settings, because your SVG file has different aspect-ratios for viewBox and for svg.width,svg.height");
                                 ...
                                 transform:(CGAffineTransform)transform
{
    ...
    if (svgGradient == nil) {
        ...
        SVGKitLogError("This SVG shape has a URL fill (%@), but could not find an XML Node with that ID inside the DOM tree (suggests the parser failed, or the SVG file is corrupt)", gradId );
    }
...
+(void) parsePreserveAspectRatioFor:(Element<SVGFitToViewBox>*) element
{
    ...
    if( stringPreserveAspectRatio.length > 0 )
    {
        ...
        for( NSString* aspectRatioCommand in aspectRatioCommands )
        {
            ...
            {
                SVGKitLogError("Found unexpected preserve-aspect-ratio command inside element's 'preserveAspectRatio' attribute. Command = '%@'", aspectRatioCommand );
            }
        }
    }
}
