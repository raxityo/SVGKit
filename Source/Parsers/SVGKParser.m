...
- (void) addParserExtension:(NSObject<SVGKParserExtension>*) extension
{
	...
	{
		SVGKitLogError("[%@] WARNING: attempted to add a ParserExtension that was already added = %@", [self class], extension);
		return;
	}
...
- (SVGKParseResult*) parseSynchronously
{
	...
	*/
	...
	/* 
	 SVGKitLogError("[%@] WARNING: Substituting entities directly into document, c.f. http://www.xmlsoft.org/entities.html for why!", [self class]);
	...
	if( ctx ) // if libxml init succeeds...
	{
		...
		while( bytesRead > 0 )
		{
			...
			if( [currentParseRun.errorsFatal count] > 0 )
			{
				...
				{
				SVGKitLogError("[%@] libXml reported internal parser error with magic libxml code = %li (look this up on http://xmlsoft.org/html/libxml-xmlerror.html#xmlParserErrors)", [self class], (long)libXmlParserParseError );
				currentParseRun.libXMLFailed = YES;
				}
				else
				{
					SVGKitLogError("[%@] SVG parser generated one or more FATAL errors (not the XML parser), errors follow:", [self class] );
					for( NSError* error in currentParseRun.errorsFatal )
					{
						SVGKitLogError("[%@] ... FATAL ERRRO in SVG parse: %@", [self class], error );
					}
				}
...
- (SVGKSource *)loadCSSFrom:(NSString *)href
{
    ...
    if( cssSource == nil )
    {
        ...
        if( cssText == nil )
        {
            SVGKitLogError("[%@] Unable to find external CSS file '%@'", [self class], href );
        }
        else
        {
...
- (void)handleStartElement:(NSString *)name namePrefix:(NSString*)prefix namespaceURI:(NSString*) XMLNSURI attributeObjects:(NSMutableDictionary *) attributeObjects
{
...
#if DEBUG_XML_PARSER
			SVGKitLogError("[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), subParser );
#endif
	...
	SVGKitLogError("[%@] WARN: found a tag with no namespace parser: (</%@>), using default parser(%@)", [self class], name, eventualParser );
...
#if DEBUG_XML_PARSER
	SVGKitLogError("[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), eventualParser );
#endif
...
static void startElementSAX (void *ctx, const xmlChar *localname, const xmlChar *prefix,
	...
	SVGKitLogError("[%@] DEBUG_VERBOSE: <%@%@> (namespace URL:%@), attributes: %i", [self class], [NSString stringWithFormat:@"%@:",stringPrefix], name, stringURI, nb_attributes );
#endif
#endif
	...
	if( prefix2 == nil )
	{
		...
		if( nb_namespaces > 0 )
		{
			...
			{
				SVGKitLogError("[%@] DEBUG: found namespace [%i] : %@", [self class], i, namespaces[i] );
			}
		}
		else
			SVGKitLogError("[%@] DEBUG: there are ZERO namespaces!", [self class] );
		 */
	}
#endif
...
- (void)handleEndElement:(NSString *)name {
	//DELETE DEBUG SVGKitLogError("ending element, name = %@", name);
...
#if DEBUG_XML_PARSER
#if DEBUG_VERBOSE_LOG_EVERY_TAG
	SVGKitLogError("[%@] DEBUG-PARSER: ended tag (</%@>), handled by parser (%@) with parent parsed by %@", [self class], name, parser, parentParser );
#endif
#endif
...
static void errorEncounteredSAX (void *ctx, const char *msg, ...) {
	SVGKitLogError("Error encountered during parse: %s", msg);
	SVGKParser* self = getCurrentlyParsingParser();
...
{
	SVGKitLogError("Error: unparsed entity Decl");
}
...
+(NSDictionary *) NSDictionaryFromCSSAttributes: (Attr*) styleAttribute {
	...
	{
		SVGKitLogError("[%@] WARNING: asked to convert an empty CSS string into a CSS dictionary; returning empty dictionary", [self class] );
